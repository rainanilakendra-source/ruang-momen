...
import { buildGuestUrl, generateQrSvg, parseQrMode, QR_MODE_DETAILS } from "../../../../lib/qr";
import { prisma } from "../../../../lib/prisma";

export const metadata: Metadata = { title: "Cetak QR — Ruang Momen" };

export default async function PrintQrPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ mode?: string | string[] }> }) {
  const user = await requireUser();
  const { id } = await params;
  const mode = parseQrMode((await searchParams).mode);
  ...
  return (
    <div className="print-preview-page">
      ...
      <PrintableQrCard eventName={event.name} guestUrl={guestUrl} qrSvg={qrSvg} modeLabel={QR_MODE_DETAILS[mode].printLabel} />
    </div>
  );
}
