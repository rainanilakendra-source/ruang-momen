...
export default async function PrintQrPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ mode?: string | string[]; source?: string | string[] }> }) {
  ...
  if (!event) notFound();
  if (!(await hasPlanFeature(user.id, PLAN_FEATURES.ADVANCED_QR))) notFound();
  ...
  return <div className="print-preview-page">
    <div className="print-controls mx-auto mb-7 flex w-full max-w-[148mm] flex-col-reverse gap-3 sm:flex-row sm:justify-between">
      <Link href={`/dashboard/ruang/${id}/qr`} className="flex min-h-12 items-center justify-center rounded-xl border border-[#F5F0E7]/12 px-6 text-sm font-semibold text-[#F5F0E7] transition hover:border-[#F5F0E7]/25 hover:bg-[#F5F0E7]/[.04]">Kembali ke QR Studio</Link>
      <PrintButton />
    </div>
