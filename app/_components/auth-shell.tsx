  ...
  return (
    <main className="relative grid min-h-screen overflow-hidden bg-[#071727] px-5 py-8 text-[#F5F0E7] sm:px-8 sm:py-12 lg:grid-cols-[minmax(0,.9fr)_minmax(28rem,1.1fr)] lg:gap-12 lg:px-12">
      ...
      <section className="relative hidden flex-col justify-between rounded-[2rem] border border-[#D6B56F]/15 bg-[#0A1D30]/70 p-10 shadow-[inset_0_1px_0_rgba(245,240,231,.025),0_24px_70px_rgba(0,0,0,.16)] lg:flex xl:p-14">
        <Link href="/" className="mx-auto w-fit rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#D6B56F]" aria-label="Kembali ke beranda Ruang Momen">
        ...
        <div className="w-full max-w-xl">
          <p className="text-xs font-bold uppercase tracking-[.22em] text-[#D6B56F]">Satu ruang bersama</p>
          <p className="mt-5 font-serif text-4xl leading-[1.35] italic text-[#F5F0E7] xl:text-5xl">Setiap sudut punya cerita yang layak disimpan.</p>
          <div className="mt-8 flex items-center gap-3 text-sm text-[#AEB8BE]"><span className="h-px w-10 bg-[#D6B56F]/55" />Ruang Momen</div>
        </div>
        <p className="text-xs text-[#AEB8BE]/65">Satu QR. Banyak sudut. Satu cerita.</p>
      </section>

      <section className="relative flex items-center justify-center py-4 lg:py-8">
        <div className="w-full max-w-[32rem]">
          <Link href="/" className="mx-auto mb-8 block w-fit rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#D6B56F] lg:hidden" aria-label="Kembali ke beranda Ruang Momen">
