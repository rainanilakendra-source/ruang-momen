...
export function PrintableQrCard({ eventName, guestUrl, qrSvg, modeLabel, sourceLabel }: { eventName: string; guestUrl: string; qrSvg: string; modeLabel: string; sourceLabel: string | null }) {
  return (
    <article className="printable-qr-card mx-auto flex aspect-[148/210] w-full max-w-[148mm] flex-col items-center overflow-hidden rounded-[1.5rem] border border-[#D6B56F]/30 bg-[#071727] px-[10%] py-[7%] text-center text-[#F5F0E7] shadow-[0_24px_70px_rgba(0,0,0,.28)]">
      ...
      {sourceLabel && <div className="mt-[2%]"><p className="text-[clamp(.42rem,1vw,.58rem)] font-bold uppercase tracking-[.18em] text-[#D6B56F]">Sudut Momen</p><p className="mt-[1%] text-[clamp(.58rem,1.4vw,.78rem)] font-semibold text-[#F5F0E7]">{sourceLabel}</p></div>}
