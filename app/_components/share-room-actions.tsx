...
export function ShareRoomActions({ eventName, eventId, eventSlug, baseUrl, variants }: { eventName: string; eventId: string; eventSlug: string; baseUrl: string; variants: Record<QrMode, QrVariant> }) {
  ...
  const printHref = `/dashboard/ruang/${eventId}/qr${query.size ? `?${query.toString()}` : ""}`;
  ...
  return <>
    ...
    <div className="mt-5"><label htmlFor="photo-source" className="text-xs font-bold uppercase tracking-[.14em] text-[#AEB8BE]">Sudut Momen</label><input id="photo-source" type="text" value={sourceInput} onChange={(event) => setSourceInput(event.target.value)} placeholder="Contoh: Meja Sahabat" className="mt-3 min-h-11 w-full rounded-xl border border-[#F5F0E7]/12 bg-[#071727]/55 px-4 text-sm text-[#F5F0E7] placeholder:text-[#AEB8BE]/60 focus:border-[#D6B56F]/40 focus:outline-none" /><p className="mt-2 text-xs leading-5 text-[#AEB8BE]">Opsional. Gunakan untuk membedakan asal momen dari QR yang berbeda.</p></div>
