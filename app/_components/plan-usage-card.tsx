...
type Usage = { guests: number; photos: number; storageBytes: number; activeDays: number };
type ActivePlan = { name: string; maxGuests: number; limit: { maxPhotos: number; maxStorageBytes: bigint; maxActiveDays: number } | null };
...
export function PlanUsageCard({ plan, usage }: { plan: ActivePlan | null; usage: Usage }) {
  ...
  const items = [
    { label: t("planLimits.photoUsage"), current: usage.photos, limit: plan.limit?.maxPhotos ?? 0, display: `${usage.photos.toLocaleString(language)} / ${(plan.limit?.maxPhotos ?? 0).toLocaleString(language)}` },
    { label: t("planLimits.storageUsage"), current: usage.storageBytes, limit: Number(plan.limit?.maxStorageBytes ?? BigInt(0)), display: `${bytesLabel(usage.storageBytes, language)} / ${bytesLabel(Number(plan.limit?.maxStorageBytes ?? BigInt(0)), language)}` },
    { label: t("planLimits.activeDaysUsage"), current: usage.activeDays, limit: plan.limit?.maxActiveDays ?? 0, display: `${usage.activeDays.toLocaleString(language)} / ${(plan.limit?.maxActiveDays ?? 0).toLocaleString(language)}` },
  ];
