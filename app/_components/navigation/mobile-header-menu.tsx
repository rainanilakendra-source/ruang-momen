...
export function MobileHeaderMenu({ variant, showAdmin = false, showSuperAdmin = false }: { variant: "public" | "dashboard"; showAdmin?: boolean; showSuperAdmin?: boolean }) {
  ...
  const dashboardSections: MobileMenuSection[] = [
    ...
    { titleKey: "navigation.sections.account", items: [{ labelKey: "navigation.profile", icon: "account" }, { labelKey: "navigation.settings", icon: "lock" }, { labelKey: "navigation.language", action: languageAction }, { labelKey: "common.help", href: "/#faq", icon: "help" }, { labelKey: "common.logout", action: logoutAction }] },
    ...(showAdmin ? [{ titleKey: "navigation.sections.adminPanel", items: [{ labelKey: "navigation.adminDashboard", href: "/admin/orders", icon: "lock" as const }, { labelKey: "navigation.adminOrders", href: "/admin/orders", icon: "billing" as const }, { labelKey: "navigation.users", icon: "user" as const }] }] : []),
    ...(showSuperAdmin ? [{ titleKey: "navigation.sections.superAdmin", items: [{ labelKey: "superadmin.dashboard", href: "/superadmin", icon: "lock" as const }, { labelKey: "superadmin.users", icon: "user" as const }, { labelKey: "superadmin.plans", href: "/superadmin/plans", icon: "billing" as const }, { labelKey: "superadmin.features", href: "/superadmin/features", icon: "album" as const }, { labelKey: "superadmin.payments", href: "/superadmin/payments", icon: "billing" as const }, { labelKey: "superadmin.settings", icon: "lock" as const }] }] : []),
  ];
