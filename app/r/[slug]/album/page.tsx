...
export default async function GuestAlbumPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const records = await prisma.photo.findMany({ where: { eventId: event.id }, orderBy: { createdAt: "desc" }, take: 100, select: { id: true, guestName: true, source: true, createdAt: true } });
  const photos = records.map((photo) => ({ id: photo.id, guestName: photo.guestName, sourceLabel: photo.source ? formatPhotoSource(photo.source) : null, uploadedAt: new Intl.DateTimeFormat("id-ID", { dateStyle: "medium", timeStyle: "short" }).format(photo.createdAt) }));
